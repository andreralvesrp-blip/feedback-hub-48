    ...
    <main className="min-h-screen bg-background pb-24">
      <header className="sticky top-0 z-20 border-b border-border bg-background/90 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-xl font-bold leading-tight">{company?.name || "Configure sua empresa"}</h1>
          </div>
          <div className="flex items-center gap-2">
            {memberships.length > 1 && <Select value={company?.id} onValueChange={switchCompany}><SelectTrigger className="hidden w-56 rounded-lg sm:flex"><SelectValue /></SelectTrigger><SelectContent>{memberships.map((item) => item.companies && <SelectItem key={item.company_id} value={item.company_id}>{item.companies.name}</SelectItem>)}</SelectContent></Select>}
      ...
      <div className="mx-auto max-w-6xl px-4 py-5">
        <Tabs defaultValue={company ? "dashboard" : "settings"} className="space-y-5">
          {memberships.length > 1 && <Select value={company?.id} onValueChange={switchCompany}><SelectTrigger className="h-12 rounded-lg sm:hidden"><SelectValue /></SelectTrigger><SelectContent>{memberships.map((item) => item.companies && <SelectItem key={item.company_id} value={item.company_id}>{item.companies.name}</SelectItem>)}</SelectContent></Select>}
          <TabsList className="grid h-auto grid-cols-5 rounded-lg bg-muted p-1">
            <TabsTrigger value="dashboard" className="rounded-lg"><BarChart3 className="h-4 w-4 sm:mr-2" /><span className="hidden sm:inline">Painel</span></TabsTrigger>
            <TabsTrigger value="responses" className="rounded-lg"><Table2 className="h-4 w-4 sm:mr-2" /><span className="hidden sm:inline">Respostas</span></TabsTrigger>
            <TabsTrigger value="budgets" className="rounded-lg"><MessageCircle className="h-4 w-4 sm:mr-2" /><span className="hidden sm:inline">Orçamentos</span></TabsTrigger>
            <TabsTrigger value="qr" className="rounded-lg"><QrCode className="h-4 w-4 sm:mr-2" /><span className="hidden sm:inline">QR</span></TabsTrigger>
            <TabsTrigger value="settings" className="rounded-lg"><Settings className="h-4 w-4 sm:mr-2" /><span className="hidden sm:inline">Config</span></TabsTrigger>
          </TabsList>

          <TabsContent value="dashboard" className="space-y-5">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end sm:gap-3"><span className="text-sm font-semibold text-muted-foreground">Período</span><div className="flex items-center gap-2"><Button variant={periodValue === "current" ? "hero" : "outline"} onClick={handleCurrentMonthClick} disabled={!company || dashboardLoading}>Mês atual</Button><Select value={selectedManualMonth} onValueChange={handleManualMonthChange} disabled={!company || dashboardLoading}><SelectTrigger className="w-52 rounded-lg"><SelectValue placeholder="Selecionar mês" /></SelectTrigger><SelectContent>{monthOptions.map((month) => <SelectItem key={month.month_start} value={month.month_start}>{month.month_label}</SelectItem>)}</SelectContent></Select>{dashboardLoading && <Loader2 className="h-4 w-4 animate-spin text-primary" />}</div></div>
            <div className={`grid gap-3 transition-opacity duration-200 sm:grid-cols-2 lg:grid-cols-7 ${dashboardLoading ? "opacity-60" : "opacity-100"}`}>
              {[["Índice de Experiência", stats.experienceIndex], ["Respostas", stats.total], ["Adorei", stats.loved], ["Foi ok", stats.ok], ["Não gostei", stats.improve], ["Orçamentos", stats.budgets], ["Google", stats.google]].map(([label, value]) => (
                <div key={label} className="rounded-lg bg-card p-4 shadow-soft"><p className="text-sm text-muted-foreground">{label}</p><p className="text-3xl font-bold">{value}</p></div>
              ))}
            </div>
          ...
          <TabsContent value="qr" className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <section className="rounded-lg bg-gradient-card p-6 shadow-soft"><p className="text-sm font-bold text-primary">Arte simples para impressão</p><h2 className="mt-3 text-3xl font-bold">Avalie sua experiência</h2><p className="mt-2 text-muted-foreground">Aponte a câmera do celular para o QR Code e responda em menos de 60 segundos.</p><div className="mt-6 rounded-lg bg-card p-3 text-sm break-all">{reviewUrl}</div>{panelUrl && <Button asChild variant="quiet" className="mt-4"><Link to={panelUrl} target="_blank"><ExternalLink className="h-4 w-4" /> Painel público</Link></Button>}</section>
          </TabsContent>
