...
const PublicPanel = () => {
  ...
  if (!metrics) {
    return <main className="grid min-h-screen place-items-center bg-background px-5 text-center"><div className="rounded-lg bg-card p-6 shadow-soft"><h1 className="text-2xl font-bold">Painel indisponível</h1><p className="mt-2 text-muted-foreground">Token inválido ou empresa não encontrada.</p></div></main>;
  }
  ...
  return (
    <main className="min-h-screen bg-background px-4 py-5">
      <div className="mx-auto max-w-4xl space-y-5">
        ...
        <div className="flex items-center gap-3 rounded-lg bg-card p-4 shadow-soft">
          <Calendar className="h-5 w-5 text-primary" />
          <Input type="month" value={month} onChange={(e) => setMonth(e.target.value)} className="h-12 rounded-lg" />
        </div>
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={String(label)} className="rounded-lg bg-card p-5 shadow-soft">
